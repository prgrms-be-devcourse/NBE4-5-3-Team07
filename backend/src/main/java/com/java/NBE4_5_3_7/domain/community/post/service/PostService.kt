...
class PostService(
    ...
    fun postLike(memberId: Long, postId: Long): LikeResponseDto {
        ...
        val lockKey = "lock:post:like:$postId"  // 콜론 추가
        val lock = redissonClient.getLock(lockKey)
        val redisKey = "post:like:$postId"  // 키 이름 통일 (likes -> like)
        val atomicLong = redissonClient.getAtomicLong(redisKey)  // getBucket 대신 getAtomicLong 사용
        ...
        try {
            ...
            val currentLikes = if (atomicLong.isExists) atomicLong.get().toInt() else likeRepository.countByPostPostId(postId) ?: 0

            return if (existingLike != null && existingLike.isPresent) {
                likeRepository.delete(existingLike.get())
                atomicLong.set(currentLikes - 1L)
    ...
    fun flushLikeCountsToDB() {
        ...
        for (key in keys) {
            ...
            try {
                ...
                // post 엔티티에 likeCount 필드가 있다면 업데이트
                setField(post, "likeCount", redisCount.toInt())
                postRepository.save(post)
            } catch (e: Exception) {
                println("Error updating likeCount for postId $postId: ${e.message}")
            }
        }
    }
}
