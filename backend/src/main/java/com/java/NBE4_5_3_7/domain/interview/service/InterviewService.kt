...
class InterviewService(
    ...
    fun showOneInterviewContent(id: Long, memberId: Long): InterviewResponseDto {
        ...
        val next = interviewRepository.findNextInterviewContent(content.interview_content_id)
        val nextId = next.map { obj: InterviewContent -> obj.interview_content_id }.orElse(null)

        return InterviewResponseDto(
            content.interview_content_id,
            content.head_id,
            content.tail_id,
            content.question,
            content.modelAnswer,
    ...
    fun showRandomInterviewContent(randomRequestDto: RandomRequestDto, memberId: Long): RandomResponseDto {
        ...
        return RandomResponseDto(
            ...
            InterviewResponseDto(
                interview.interview_content_id,
                interview.head_id,
                interview.tail_id,
                interview.question,
    ...
    fun showMyBookmark(memberId: Long): List<BookmarkResponseDto> {
        ...
        val bookmarks = member.bookmarks
        val result: MutableList<BookmarkResponseDto> = ArrayList()

        for (bookmark in bookmarks) {
            result.add(
                BookmarkResponseDto(
                    bookmark.id,
                    bookmark.interviewContent.question,
                    bookmark.interviewContent.modelAnswer
                )
            )
        }
